...
#
# 用法:
#   bash scripts/run_tensorboard.sh                                          # 看 checkpoints/ 下所有实验 (多 run 对比)
#   bash scripts/run_tensorboard.sh checkpoints/offpolicy_short128_forward_kl # 只看一个实验
#   PORT=6007 bash scripts/run_tensorboard.sh                                 # 换端口
#
# 本机浏览器访问 (先在本机开一个终端):

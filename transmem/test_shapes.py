...
def test_forward_backward():
    ...
    hq_prime = mem.correct(hq_stu, proposal)
...
def test_all_queries_causality():
    ...
    with torch.no_grad():
        output_all = mem(X, return_all_queries=True)
        ms_all = output_all.ms                                   # [B, M, D]
        ...
        X1 = X.clone(); X1[:, N, :] += 1.0
        ms1 = mem(X1, return_all_queries=True).ms
        ...
        X2 = X.clone(); X2[:, -1, :] += 1.0
        ms2 = mem(X2, return_all_queries=True).ms
    ...
    with torch.no_grad():
        ms0 = mem0(X, return_all_queries=True).ms
...
def test_kv_cache_matches_full():
    ...
    for pos in ("none", "rope", "learned"):
        ...
        with torch.no_grad():
            full = mem(X, return_all_queries=True).ms            # [1, M, D]
            past = DynamicCache()
            steps = [mem(X[:, :N + 1, :], past_key_values=past, use_cache=True).ms]
            for i in range(1, M):
                steps.append(mem(X[:, N + i:N + i + 1, :],
                                 past_key_values=past, use_cache=True).ms)
...
def test_trailing_padding_invariance():
    ...
    with torch.no_grad():
        ms = mem(X, return_all_queries=True).ms                  # [1, M, D]
        ms_pad = mem(X_pad, return_all_queries=True).ms[:, :M, :]  # 取前 M 个有效位
...
def test_real_config():
    ...
    proposal = mem(X)
    assert proposal.ms.shape == (B, D)
    mem.correct(X[:, -1, :], proposal).pow(2).mean().backward()
    ms = proposal.ms
